...
/* ---------------------------------------------------------------------------
   ...
   Codes: P = Local · T = Tour (Ex/Outstation) · WFH = Work From Home
          L = Leave · HO = Holiday · S = Sunday/Weekly off · A = Marked Absent · blank = Absent
--------------------------------------------------------------------------- */
...
export default function AttendanceSheet() {
  ...
  const codeFor = (u, dateStr, dow) => {
    ...
    if (s) {
      if (s.marked_absent) return "A";              // admin/HOD marked absent
    ...
    <div style={{ padding: "0 4px 40px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button className="btn" style={{ background: "#22a45d", color: "#fff", borderColor: "transparent" }} onClick={load} disabled={busy}>{busy ? "Loading…" : "Show"}</button>
      ...
      {!shown ? (
        <div style={{ background: "#fff", borderRadius: 14, boxShadow: "var(--shadow)", padding: 50, textAlign: "center", color: "var(--muted)" }}>
          Select month and filters, then click <b>Show</b> to load the attendance sheet.
        </div>
      ) : (
      <div style={{ background: "#fff", borderRadius: 14, boxShadow: "var(--shadow)", overflow: "hidden" }}>
        <div style={{ overflowX: "auto", maxHeight: "70vh", overflowY: "auto" }}>
          <table style={{ borderCollapse: "collapse", fontSize: 11.5, minWidth: 900 }}>
            ...
            <tbody>
              {grid === null ? (
                <tr><td colSpan={daysInMonth + 9} style={{ padding: 30, textAlign: "center", color: "var(--muted)" }}>Loading…</td></tr>
              ) : grid.length === 0 ? (
                <tr><td colSpan={daysInMonth + 9} style={{ padding: 30, textAlign: "center", color: "var(--muted)" }}>No users</td></tr>
                ...
                <tr key={u.id} style={{ borderTop: "1px solid #eef1f8" }}>
                  ...
                  <td style={{ ...tot, color: "#c03636" }}>{totals.a}</td>
                  <td style={{ ...tot, fontWeight: 900, background: "#eef1ff" }}>{totals.total}</td>
                </tr>
              ))}
            </tbody>
          </table>
